...
# 3x5 digit bitmaps, top row first. Bit 2 = left pixel. (Same as tiles.c.)
DIGITS = {
...
def pack_quad(cell, ox, oy):
    """8x8 block -> 16 bytes 2bpp. Mirrors pack_quadrant()."""
    out = []
    for y in range(8):
        lo = hi = 0
        for x in range(8):
...
def pack_margin(left):
    """2px vertical frame line. Mirrors pack_margin()."""
    out = []
    for _ in range(8):
        lo = hi = 0
        for x in range(8):
...
def main():
    ...
    with open("src/tiles_gen.c", "w") as f:
        ...
        for t in CURSOR:
            f.write("    " + ", ".join(f"0x{b:02X}" for b in t) + ",\n")
        f.write("};\n")
    print(f"Wrote {len(tiles)} grid tiles + 4 cursor tiles to src/tiles_gen.c")
