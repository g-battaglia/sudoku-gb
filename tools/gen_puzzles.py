...
def main():
    ...
    levels = []  # (difficulty, givens_str, solution_str)
    for diff_name, count, givens_target in DIFFICULTIES:
        for _ in range(count):
            ...
            levels.append((diff_name, to_str(puzzle), to_str(solution)))

    with open("src/puzzles_gen.c", "w") as f:
        f.write("/* GENERATED by tools/gen_puzzles.py -- DO NOT edit by hand. */\n")
        f.write("/* Givens + full solution per level. The solution powers the\n")
        f.write(" * HINT command (START menu); it costs 81 bytes per level. */\n")
        f.write('#include "puzzles.h"\n\n')
        f.write("const Puzzle puzzles[LEVEL_COUNT] = {\n")
        for diff_name, givens, solution in levels:
            f.write(f'    {{ DIFF_{diff_name}, "{givens}", "{solution}" }},\n')
        f.write("};\n")
