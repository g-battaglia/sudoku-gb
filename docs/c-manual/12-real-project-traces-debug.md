...
5. `tests/test_host.c` — executable documentation. Each `test_*` plays with the board and `assert`s a rule (validity, load, conflicts, saturation, hint lock, bitmap, restore, save-format). If you change `board.c`, this file is the specification.
6. `src/save.h`, then `src/save.c` + `src/save_format.{h,c}` — the slot struct, latch discipline, fixed offsets (`SAVE_OFF_*`), magic + version + checksum + field validation (the format half is hardware-free and host-tested).
7. `src/input.h` + `src/input.c` — edge + 16-bit-timer repeat + combo (one idea per function, chapter 11 §9).
8. `src/main.c` — the conductor: one `switch`, one handler per state (`game_update` splits nav/editing). Owns the `Cursor`, `Preview` (`pv`) and `Pending` structs plus the save flow; draws nothing directly — every visual change is a `ui_*` call.
9. `src/ui.h`, then `src/ui.c` — all drawing. Header comment first (double-buffer design, chapter 11 §4), then one screen function at a time plus the delta helpers. Status lines take plain values (mistakes passed in, never read from the board).
...
```c
for (i = BOOT_VSYNCS; i != 0; i--) vsync();   /* SGB PAL warmup, harmless elsewhere */
...
cursor.row = cursor.col = 0; cursor.entry = 1; cursor.editing = 0;
pend.need_win = pend.need_saved = 0; frame = flash = 0;
preview_forget();     /* pv.row = PV_NONE = "no preview tracked" (sentinel, ch.04) */
marks_clear(marks);   /* explicit: no reliance on zero-init */
has_save = save_read(&slot);        /* validate header + field ranges (ch.10) */
...
4. The handler reads `input_pressed` (single actions) or `input_dir` (movement), mutates state first (`board_set`, `cursor`, `menu_choice`), then calls `ui_*` drawing second.
...
## 4. Trace C — editing a digit (the modal core)

On a free cell, `A` calls `enter_editing()`: locked ⇒ `locked_feedback()` (cursor hidden `FLASH_LOCKED` frames — sprite blink, zero tile writes); free ⇒ `cursor.entry = current ? current : 1` (editing starts from the digit already there, or 1 if empty — resuming an edit never loses the visible value), `cursor.editing = 1`, cursor re-shown. From now on Up/Right and Down/Left change `cursor.entry` instead of moving the cursor (`entry % 9 + 1` wraps 9→1; `(entry + 7) % 9 + 1` wraps 1→9 — the 1-based wrap from chapter 04 §4).

Every frame, `preview_update()` blinks the picked digit with `ui_preview(row, col, value, show)` — **the board is never touched by the blink**, only tiles. The tracker (`pv.row/pv.col/pv.val/pv.shown`, `PV_NONE` = untracked) reconciles three disturbance sources: mode exit (`want` false → erase), cursor/value change (tracker mismatch → erase + re-track), blink phase (`frame >> BLINK_SHIFT & 1` — bit 5 of the 16-bit free-running counter, chapter 07 TEST idiom — differing from `pv.shown` → one tile write). Maximum one tile operation per frame; usually zero.

`A` (`confirm_editing` via `try_place_digit`, full body in chapter 04 §5): tentative `board_set`, `board_conflicts` check — illegal ⇒ restore + `ui_cell` redraw + `preview_forget` (the redraw already killed any visible preview, so *forget*, not *erase* — the distinction from §2) + `board_add_mistake` + cursor-hide `FLASH_REJECT` frames, *keep picking* (no game over, mistakes tallied); legal ⇒ `cursor.editing = 0`, `preview_forget`, `ui_cell`, `board_is_solved` ⇒ `win_now` (which defers the win screen to state entry — see §5 why). `B` (`cancel_editing`) erases the preview; the board is untouched. `B` outside editing (`erase_cell`) clears unlocked player digits in place; locked non-empty cells blink instead of silently ignoring (every input gets feedback — a UI rule worth stealing).

## 5. Trace D — START menu, HINT, SAVE, win, screen swap

`START` in game: `cursor.editing = 0`, `preview_erase` (a preview may be visible — erase, not forget), `flash = 0`, `menu_choice = 0`, `ui_pause(...)` (status: level-within-mode, difficulty, mistakes passed as a value + 5 items + 3-line help), `state = ST_PAUSE`. `pause_update` is a `switch`: Up/Down wrap through RESUME/HINT/SAVE/PLAY AGAIN/MENU (`ui_pause_cursor` = marker-pair delta on the visible map), B/START resumes, A dispatches. `saved_update` (confirmation screen) returns on any of A/B/START — generous exit keys on dead-end screens.

`do_hint()`: cursor cell if free + editable, else first such cell (linear scan, `PV_NONE` sentinel = full ⇒ `resume_game`, since a full grid already triggered the win path); `value = puzzle_solution(level, idx)` (the ROM always knew the answer — HINT is a ROM read plus a lock); `board_set` + `board_reveal` (gray like player, locked like clue — the third origin from chapter 07 §2); solved ⇒ `win_now`, else `resume_game` (full atomic redraw shows the digit; HINT is rare, so simplicity beats a delta here).

`save_store(active)`: snapshot level + `board_get`/`board_origin` × 81 + mistakes + marks into `slot` (hand loops, not `memcpy` — keeps the helper out of ROM), `save_write(&slot)` (latch + checksum, chapter 10 §5), `has_save = 1`. SAVE ⇒ `active = 1` + `ST_SAVED` confirmation; every win (`win_now`: `marks_set` + `save_store(0)` + hide cursor + record into `pend`) ⇒ `active = 0` so completions persist even without explicit save. `apply_load` re-reads SRAM (sees saves from *this* session too; ranges already validated by `save_read`), restores marks, then branches: `game_active` ⇒ `board_restore` + first-editable cursor + game screen via shared `enter_game_state()`; else ⇒ select screen of the saved difficulty positioned at the saved level (`sel_page`/`sel_row` derived — the marks-only resume).

The deferred-draw trick (`pend.need_win`/`pend.need_saved` in the `Pending` struct): win/save screens are drawn on *state entry* (flat in `main`'s switch), never nested inside the game/pause update — drawing a full screen mid-handler garbled menu text during development, so `win_now` only *records* (`pend.win_level`, `pend.win_last`, flag) and the switch *presents* (passing `board_errors()` at present time). Symptoms of violating this: text from two screens interleaved. Any future full-screen transition from inside a handler must use the same deferral.
...
`win_now` → `ST_WIN` → (next frame entry) `ui_win(pend.win_level, pend.win_last, board_errors())` → `win_update`: A/START advances to `level + 1` via `start_level` (fresh board, same flow) or, on the mode's last level (`pend.win_last`), back to select. Every win rewrote the slot already (`save_store(0)` inside `win_now`), so powering off on the win screen loses nothing — persistence precedes presentation. The level number shown is *within the mode* (`level_in_diff()`), matching the select screen's numbering; global 0–299 indices never reach the player. Two numberings (storage vs display) with one conversion function — the pattern for any indexed content.
...
1. **Drawing to the wrong map** — content on the wrong screen or never flipping. Check `use_hidden_map` routing: full screens hidden, deltas visible (chapter 11 §4).
...
5. **Hardware include in portable modules** — `#include <gb/gb.h>` sneaks into `board`/`puzzles`/`save_format`, `make test-host` fails on PC. The boundary is the feature (chapter 09 §3 exclusion list).
6. **Deferred-draw violation** — interleaved text from two screens. Full screens only on state entry via `pend.need_*` flags (§5).
...
- **Help text / menu layout**: edit `draw_*_content()` in `ui.c`. Keep lines ≤ 20 columns; numbers via `draw_dec3`/`draw_num2`, markers via `marker_pair_set()`; stop strings at `\0`; full screens via begin → draw → OAM → present.
...
- **Controls**: edit the `*_update()` handler in `main.c` (`game_update_nav` vs `game_update_editing` for the two cursor modes). Single actions → `input_pressed`; movement → `input_dir`; global chords before the state switch.
